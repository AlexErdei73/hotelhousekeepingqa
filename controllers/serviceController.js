...
exports.services_upload_post = function(req, res, next) {
    ...
    _getServices(fileBuffer.toString("utf-8").split('\n'), date, (err, services) => {
        if (err) {
            return next(err);
        }
        const saveFunctions = [];
...
function _getServices(lines, date, cb) {
    ...
    async.parallel({
        ...
        cb(null, services.map(service => {
           const newService = { 
            date: service.date,
            type: service.type,
            cleaner: cleaners.find(cleaner => cleaner.name_id === service.cleanerName.toLowerCase())._id,
...
function _getDate(fileName) {
    const date = fileName.split('-')[2].slice(1,11);
